// Fase_3/Convolution/conv_chained.cu
//
...
// "Plan de Precision Mixta", seccion 02 -- esa es la referencia normativa.
//
...
// cualquier resultado -- ver el comentario junto a gpu_fp64_step().
...
// cero. Mismo patron que Fase_4/Stencil. Dejar comp en 0 tras la siembra de
// X0 descartaria el redondeo x0->T de la primerisima conversion, que se
// propagaria a traves de conv(*, W) en cada iteracion en vez de quedar
// capturado desde el principio, como exige la propiedad de reconstruccion
// Q(v)+comp=v que el resto del mecanismo asume ya valida desde t=0 -- ver
// Fase_3/GEMM/gemm_chained.cu para el razonamiento completo, incluyendo el
// impacto en el gate K=1 de Fase 4.
template <typename T>
...
// Un paso encadenado: X_out = conv(X_in, W) (+ correccion si comp_on).
// Sin ancla FP64 -- Fase 3, ver el mismo comentario en gemm_chained.cu.
template <typename T>
