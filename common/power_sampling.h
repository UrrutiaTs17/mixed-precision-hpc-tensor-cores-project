// common/power_sampling.h
//
// Telemetria de energia GPU (NVML) y CPU (RAPL) para los tres kernels del
// proyecto. Origen: old/Fase_3/Stencil/tools/power_sampling.h -- portado sin
// cambios de logica, solo generalizado de "Stencil" a "los tres kernels" en
// los comentarios, porque el mecanismo no tiene nada especifico de Stencil.
//
// Opt-in por macro: sin -DUSE_NVML_TELEMETRY, NVML nunca se inicializa y las
// funciones de energia GPU devuelven datos invalidos (NaN en el CSV), pero
// el binario compila y corre igual -- util para desarrollo local sin GPU
// A100/H100, o sin permisos de NVML.
#pragma once
...
// La energia GPU se mide con DOS lecturas puntuales del contador monotonico
// de NVML (nvmlDeviceGetTotalEnergyConsumption) al abrir y cerrar cada
// segmento -- no con muestreo periodico en un hilo aparte. Un muestreador
// necesita al menos dos despertares DENTRO de la ventana medida para
// integrar algo, y a los tamanos de problema de este proyecto la ventana
// (0.2-2.1 ms en el caso mas chico) suele ser mas corta que un solo
// intervalo de muestreo -- el muestreador por hilo devolvia NaN en la
// mayoria de las corridas. Un delta de contador no necesita ninguna lectura
// "dentro" de la ventana: una antes, una despues, resta.
//
...
// dura uno o dos, porque ahi el error de cuantizacion es del orden del
// propio valor medido -- e incluso puede caer entera entre dos saltos y
// devolver 0 J con la lectura marcada como valida.
//
...
//     33.6 ms -> 4.306 J);
//   - la potencia implicita del MISMO kernel varia 2x segun la longitud de
//     la ventana (GPU_FP32: 139 W a 27.6 ms frente a 75 W a 114.7 ms).
// No es un defecto de este codigo, del handle NVML ni de la exclusion de
// checkpoints: es el paso de cuantizacion del contador.
//
// MITIGACION: es de PROTOCOLO, no de instrumentacion. Se mide sobre
// ventanas largas (ver kEnergyWindowReliableSeconds) y se normaliza por
// iteracion; los tramos cortos se emiten marcados como no fiables en vez de
// promediarse. En particular NO se vuelve al muestreo periodico en hilo:
// leeria el MISMO sensor, con el mismo refresco, y reintroduciria los NaN
// en ventanas cortas sin ganar resolucion real.
struct PowerBuffer {
  nvmlDevice_t device;
  bool nvml_enabled;
  bool segment_active;      // true entre start_sampling y stop_sampling
  bool capture_failed;      // una lectura fallo o la guarda de monotonia salto
  bool capture_has_data;    // al menos un delta begin/end se acumulo desde el ultimo clear
  unsigned long long segment_begin_mj;  // contador de energia al abrir el segmento
  unsigned long long segment_begin_ns;  // reloj de pared al abrir el segmento
  double accumulated_j;         // total corriente de energia de los tramos cerrados
  double accumulated_window_s;  // total corriente de duracion (reloj de pared) de los tramos cerrados
};
...
// completo, y con n tramos el error absoluto es de ~n saltos: solo se
// diluye si la ventana total crece con n. De ahi el criterio de
...
// queda en <= ~5%. Por debajo de ~10 refrescos (~250 ms) ese error pasa del
// 10% y la medicion deja de servir para comparar formatos entre si, que es
// justo para lo que existe energy_gpu_j_per_iter. Sin checkpointing hay un
// unico tramo y el criterio se reduce a "la ventana dura al menos 500 ms".
...
// Resultado consolidado de una ventana de medicion (una ruta, un tamano, un
// numero de iteraciones). window_reliable resume el criterio de arriba;
// energy_j/edp son alias historicos para el CSV opcional --csv que ya
// existia antes de que se agregaran los campos mas explicitos
// (energy_gpu_j, edp_j_s).
...
static bool telemetry_nvml_enabled() { return telemetry_nvml_enabled_flag(); }

static nvmlDevice_t telemetry_nvml_device() { return telemetry_nvml_device_ref(); }
...
// invalidez previo. Se llama tras el warm-up y entre tramos de energia
// (checkpoints) para que cada tramo arranque desde cero.
...
// Abre un segmento medido: lee el contador de energia acumulada de NVML y
// el reloj de pared una sola vez cada uno, y los guarda como marca de
// inicio. No hay hilo que arrancar; el segmento se cierra con una segunda
// lectura puntual de ambos en power_buffer_stop_sampling.
...
// Cierra el segmento medido: lee el contador y el reloj de pared una
// segunda vez, calcula los deltas contra las marcas de inicio de ESTE
// segmento y los acumula. Guarda de monotonia sobre el contador de
// energia: si la lectura final es menor que la inicial, el segmento se
// marca invalido en vez de acumular un delta negativo.
static void power_buffer_stop_sampling(PowerBuffer* pb) {
  ...
  if (end_ns > pb->segment_begin_ns) {
    pb->accumulated_window_s += static_cast<double>(end_ns - pb->segment_begin_ns) / 1e9;
  }
#ifdef USE_NVML_TELEMETRY
  unsigned long long energy_mj = 0;
  if (nvmlDeviceGetTotalEnergyConsumption(pb->device, &energy_mj) != NVML_SUCCESS) {
  ...
  if (energy_mj < pb->segment_begin_mj) {
    ...
    if (!warned) {
      std::fprintf(stderr,
                   "ADVERTENCIA: el contador de energia NVML no fue monotono dentro "
                   "del segmento; energia GPU sera NaN para ese tramo.\n");
      warned = true;
    }
    pb->capture_failed = true;
    return;
...
// begin/end que power_buffer_energy_joules: permite que el tiempo usado
// como denominador de avg_power_w/edp_j_s provenga del mismo intervalo que
// la energia, en vez de un reloj de pared aparte.
...
// Enumera /sys/class/powercap y suma la energia de todos los dominios
// intel-rapl:N de NIVEL SUPERIOR (excluye subdominios intel-rapl:N:M --
// core/uncore/dram -- que ya estan incluidos en el total del paquete que
// los contiene).
...
static double rapl_energy_delta(const RAEnergySnapshot& before, const RAEnergySnapshot& after) {
...
// Combina una captura de PowerBuffer (GPU) y un par de snapshots RAPL (CPU)
// en un EnergyMeasurement listo para escribir a CSV.
static EnergyMeasurement make_energy_measurement(const PowerBuffer* pb,
                                                  ...
                                                  double time_total_s, double flops_total) {
  EnergyMeasurement result;
  result.time_total_s = time_total_s;
  result.gpu_valid = power_buffer_capture_valid(pb);
  result.cpu_valid =
      rapl_before.valid && rapl_after.valid && rapl_after.energy_j >= rapl_before.energy_j;
  if (result.gpu_valid) {
  ...
  if (result.gpu_valid && result.cpu_valid) {
    ...
    result.joules_per_gflop =
        (flops_total > 0.0) ? result.energy_total_j / (flops_total / 1e9) : 0.0;
  }
