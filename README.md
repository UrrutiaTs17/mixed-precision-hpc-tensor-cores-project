...
4. Para lanzar la campaña:
   - En un clúster con SLURM: `bash run_full_pipeline_pacca.sh` — envía cada fase como un job independiente encadenado con `--dependency=afterok` (`DRY_RUN=1` imprime el grafo sin enviar nada). Es el camino recomendado: no exige tener la GPU reservada durante decenas de horas seguidas. También integra la campaña de variabilidad (réplicas para el ANOVA) como etapa aparte, con su propio post-proceso — ya preconfigurada con energía fiable en GEMM/Convolución y con las dos tandas de Stencil (numérica y de energía); no hace falta pasarle nada encima.
   - En una máquina propia: `bash run_full_pipeline.sh` corre las cuatro fases en secuencia y termina con la estadística y el Frente de Pareto 3D. **Su default es la campaña completa**; para una prueba rápida, `PIPELINE_MODE=smoke`. La campaña de variabilidad no corre desde este script (exige SLURM por diseño — cada réplica es un job independiente).
   - **Pase de energía (`RUN_ENERGY_PASS`, default `1` en ambos — ya viene activado, no hace falta pedirlo)**: una sola pasada con los `ITERS_LIST` por defecto de cada `.sbatch` deja `energy_window_reliable=0` en casi todas las filas — confirmado en producción (jobs 6925-6928/6866/6890/6867/6893). La causa: con `CHECKPOINT_EVERY>0` la ventana NVML se fragmenta en tramos, y aun forzando `CHECKPOINT_EVERY=0` (`RUN_KIND=energy`) todas las rutas de GEMM/Convolución (y las `WMMA_*_SP` de Stencil) cierran igual 2 tramos por el disparo incondicional de `es_checkpoint()`/`checkpoint_due()` en la última iteración — umbral real 1000 ms, no 500. Activar el pase de energía manda una **segunda** invocación por Fase 3/4 y kernel con `RUN_KIND=energy` y un `ITERS_LIST` grande, dimensionado para cruzar ese umbral en la ruta más rápida del barrido (la que manda, porque todas las rutas de una invocación comparten `--iters`) usando el **mínimo** `t_iter_ms` observado en producción, no la mediana — el reloj de la GPU no es estable entre invocaciones cortas seguidas (se midió hasta 40% de diferencia dentro del mismo job), y dimensionar con la mediana subestima el caso real.
     - **Por qué un solo `ITERS_LIST` compartido entre todos los tamaños de un kernel, y no uno por tamaño**: el tamaño más chico del barrido es el que necesita más iteraciones (es el más rápido por iteración); a tamaños mayores el mismo valor sobra — cruza el umbral antes, sin perder confiabilidad, solo con algo más de tiempo de GPU. Ajustar `ITERS_LIST` por tamaño reduciría ese costo, pero complica el orquestador y no hacía falta para que la campaña sea correcta; queda como mejora futura, no como pendiente de corrección. Ver el comentario de cabecera de `run_full_pipeline_pacca.sh` para los números exactos por kernel y su margen.
