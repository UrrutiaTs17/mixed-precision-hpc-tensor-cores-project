...
**NOTA MIGRACION:** este `.sbatch` compila `stencil_tc` sin `-O3` — asi estaba en el original (`old/Fase_2/Stencil/run_stencil_tc.sbatch`), a diferencia del de Fase 1, que si usa `-O3`. Se conserva igual para no alterar comportamiento ya migrado, pero es sospechoso: sin `-O3` la referencia "CPU FP32 serial" (host, escalar) corre mucho mas lenta — medido en este entorno, ~25x mas lenta que con `-O3` a igualdad de tamano/maquina — lo que infla artificialmente todos los "Speedup vs CPU" que imprime el binario. No afecta la correctitud numerica (los campos de error son identicos con y sin `-O3`), solo el rendimiento reportado de la referencia CPU. Ver el comentario junto a la linea de compilacion en el `.sbatch`.
