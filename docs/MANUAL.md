...
### Limitaciones conocidas de los gates de ancla (sin resolver, 2026-09-10)

`validacion_preliminar.sbatch` corrida real (job 6840, PACCA, `paccaA100`) encontró 2 fallas que siguen abiertas. Documentadas aquí para no relanzar la campaña completa a ciegas; **los gates NO se modificaron** — esta es una constancia, no una corrección.

1. **Stencil, `gate3_ancla.py`, K=1**: `WMMA_FP16_SP` no da exactamente el mismo `rel_l2`/`rel_linf` que `GPU_FP64` en la misma corrida (`rel_l2=2.272e-08`, `rel_linf=3.666e-08`, el gate exige que no supere 10x el `0.000e+00` de referencia). Causa identificada en el código: el buffer que se compara (`d_out_fp32_aligned`, ver la funcion que arma el checkpoint alrededor de la iteracion de ancla) es `float`, incluso en una iteración de ancla donde el cálculo interno fue exacto en `double` (`d_out64_aligned`) — se angosta a `float` solo para el reporte. La referencia `GPU_FP64` nunca pasa por ese angostamiento (`record_checkpoint_fp64` lo evita a propósito, ver su comentario de cabecera). Es coherente con lo que ya describe este manual arriba (el CSV de Stencil compara el acumulador FP32, no el estado de 16 bits) — el punto es que el gate exige `0.0` exacto en vez de una cota a escala de épsilon de `float` (~1e-7), que es lo máximo que esa comparación puede dar aunque el ancla esté perfecta. **No parece ser un bug del mecanismo del ancla**, sino un umbral del gate mal calibrado para su propio diseño de comparación.
2. **GEMM/Convolución, `gate4_medicion.py`, chequeo C** (¿`_comp` se vuelve más lenta al activar el ancla?): a la escala minúscula del gate (`GATE_N=256`/`GATE_HW=64`) fallan los 4 combos formato×kernel. Repetido a escala de campaña real (`N=1024`, `HW=256`, jobs 6854/6855 de diagnóstico): FP16 pasa limpio en ambos kernels (1.10x–1.12x); BF16 en Convolución queda justo debajo del margen del 10% (sube 9.8%, no alcanza); BF16 en GEMM sigue bajando (no solo por margen). El ruido de reloj/arranque entre las dos invocaciones separadas (K=0 y K=1) es alto a esta escala (`_none` varía hasta 21% entre invocaciones) y decrece según crece N, así que gran parte del problema es de relación señal/ruido del gate, no necesariamente del mecanismo — pero GEMM-BF16 sigue siendo una anomalía real sin explicar, a vigilar en los datos de la campaña (réplicas reales a N grande).

**Decisión tomada** (2026-09-10): lanzar Fase 1-3 normalmente (no usan ancla, no están afectadas) y Fase 4 con `SKIP_VALIDACION=1` (sin pasar por estos 2 gates), dejando esta nota como registro para la revisión con el director. Antes de reportar cualquier cifra de tiempo/energía de Fase 4 en la tesis, revisar si estos 2 puntos ya se resolvieron.
